...
def showVersion():
    try:
        try:
            resp = requests.get('https://raw.githubusercontent.com/threatcode/fuzzlist/main/get_parameters/__init__.py',timeout=3)
        except:
            write('Current get_parameters version '+__version__+' (unable to check if latest)\n')
        if __version__ == resp.text.split('=')[1].replace('"',''):
            write('Current get_parameters version '+__version__+' ('+colored('latest','green')+')\n')
        else:
            write('Current get_parameters version '+__version__+' ('+colored('outdated','red')+')\n')
    except:
        pass
...
def showBanner():
    ...
    write(colored(r"                ' by @Xnl-h4ck3r '              v" + __import__('get_parameters').__version__))
    write("")
    showVersion()
...
def clean_body(body):
    try:
        ...
        def conditional_remove(match):
            return "BASE64_REPLACED_BY_GET_PARAMETERS" if len(match.group(0)) > 10000 else match.group(0)
        # Remove matches
...
def handler(signal_received, frame):
    ...
    if stopProgram is not None:
        ...
        writerr(
            colored(
                getSPACER('>>> "Oh my God, they killed Kenny... and get_parameters!" - Kyle'),
                "red",
            )
        )
        writerr(
...
def getConfig():
    ...
    try:
        ...
        get_parametersPath = (
            Path(os.path.join(os.getenv('APPDATA', ''), 'get_parameters')) if os.name == 'nt'
            else Path(os.path.join(os.path.expanduser("~"), ".config", "get_parameters")) if os.name == 'posix'
            else Path(os.path.join(os.path.expanduser("~"), "Library", "Application Support", "get_parameters")) if os.name == 'darwin'
            else None
        )
        get_parametersPath.absolute
        if args.config is None:  
            if get_parametersPath == '':
                configPath = 'config.yml'
            else:
                configPath = Path(get_parametersPath / 'config.yml')
        else:
        ...
        if sys.stdin.isatty():
            ...
            if not stdinMultiple:
                ...
                if stdinMultiple and not burpFile and not zapFile and not caidoFile:
                    ...
                    writerr(
                        colored(
                            "You cannot pass a Burp, ZAP or Caiod file via <stdin>. Please call get_parameters by itself and provide the file with -i",
                            "red",
                        )
                    )
                    sys.exit()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="xnlLinkFinder (v" + __import__('get_parameters').__version__ + ") - by @Xnl-h4ck3r"
    )
    parser.add_argument(
        "-i",
        "--input",
    ...
    parser.add_argument(
        "--config",
        action="store",
        help="Path to the YML config file. If not passed, it looks for file 'config.yml' in the same directory as runtime file 'get_parameters'.",
    )
    parser.add_argument(
        "-nwlpl",
        "--no-wordlist-plurals",
    ...
    if args.version:
        write(colored('get_parameters - v' + __import__('get_parameters').__version__,'cyan'))
        sys.exit()
